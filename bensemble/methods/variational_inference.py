...
from ..core.base import BensembleModule


def replace_with_bayesian_layers(
    model: nn.Module, prior_sigma: float = 1.0, init_sigma: float = 0.1
) -> None:
    """
    Replaces all nn.Linear layers with BayesianLinear inplace.
    """
    for name, child in model.named_children():
        if isinstance(child, nn.Linear):
            ...
            with torch.no_grad():
                ...
                if child.bias is not None and new_layer.b_mu is not None:
                    new_layer.b_mu.data.copy_(child.bias.data)

            setattr(model, name, new_layer)
        else:
            replace_with_bayesian_layers(child, prior_sigma, init_sigma)


def compute_kl_divergence(module: nn.Module) -> torch.Tensor:
    """
    Recursively calculates KL-divergence of all bayesian layers in a model.
    """
    kl = 0.0
    for child in module.children():
        if hasattr(child, "kl_divergence"):
            ...
            kl += compute_kl_divergence(child)
    return kl


# TODO: move this class to /layers
...
# TODO: move this class also somewhere
...
class VariationalEnsemble(BensembleModule):
    """
    Реализация метода Variational Inference (Bayes by Backprop).
    Это и есть ваша "Обертка" (Wrapper).
    """
    ...
    def __init__(
        ...
        replace_with_bayesian_layers(self.model, prior_sigma=prior_sigma)
    ...
    def training_step(self, batch, batch_idx):
        ...
        kl = compute_kl_divergence(self.model)
        loss = nll + (kl / self.hparams.num_training_samples)
        self.log("train_loss", loss)
        return loss

    def configure_optimizers(self):
        return self.hparams.optimizer_cls(
            self.parameters(),
    ...
    def predict(
        ...
        """
        Makes predictions with Monte Carlo sampling.
        Returns (mean, std).
        """
        self.eval()
        ...
        self._set_sampling_mode(self.training)

        return mean.cpu(), std.cpu()

    def _set_sampling_mode(self, active: bool):
        """
        Turns on or turns off stochasticity in bayesian layers.
        """
