...
class VariationalRenyi(BaseBayesianEnsemble):
    ...
    def __init__(self, model: nn.Module, alpha: float = 1.0,
                 prior_sigma: float = 1.0, initial_rho: float = -2.0, **kwargs):
        """
        Args:
    ...
    def _make_bayesian(self, model: nn.Module) -> nn.Module:
        ...
        for module in model.modules():
            if isinstance(module, nn.Linear):
                ...
                module.weight_rho = nn.Parameter(torch.full_like(module.weight.data, self.initial_rho))
                if module.bias is not None:
                    module.bias_mu = nn.Parameter(module.bias.data.clone())
                    module.bias_rho = nn.Parameter(torch.full_like(module.bias.data, self.initial_rho))
    ...
    def _compute_log_weights(self, batch_X: torch.Tensor, batch_y: torch.Tensor) -> torch.Tensor:
        ...
        if output.dim() == 1 or output.shape[1] == 1:  # регрессия
            ...
            log_likelihood = torch.distributions.Normal(output.squeeze(), likelihood_sigma).log_prob(batch_y.squeeze())
        else:  # классификация
        ...
        for module in self.bayesian_model.modules():
            if hasattr(module, 'weight_mu'):
                log_prior += torch.distributions.Normal(0, self.prior_sigma).log_prob(module.weight).sum()
                if hasattr(module, 'bias_mu') and module.bias_mu is not None:
                    log_prior += torch.distributions.Normal(0, self.prior_sigma).log_prob(module.bias).sum()
        ...
        for module in self.bayesian_model.modules():
            if hasattr(module, 'weight_mu'):
                ...
                log_variational += torch.distributions.Normal(module.weight_mu, sigma_w).log_prob(module.weight).sum()
                if hasattr(module, 'bias_mu') and module.bias_mu is not None:
                    sigma_b = torch.log1p(torch.exp(module.bias_rho))
                    log_variational += torch.distributions.Normal(module.bias_mu, sigma_b).log_prob(module.bias).sum()

        return log_likelihood.sum() + log_prior - log_variational

    def fit(self, train_loader: torch.utils.data.DataLoader,
            val_loader: Optional[torch.utils.data.DataLoader] = None,
            num_epochs: int = 100, lr: float = 5e-4, n_samples: int = 10,
            grad_clip: Optional[float] = 5.0, **kwargs) -> Dict[str, List[float]]:
        ...
        for epoch in range(num_epochs):
            ...
            for batch_X, batch_y in train_loader:
                ...
                if self.alpha == 1.0:
                    ...
                    loss = -(1/(1-self.alpha)) * torch.logsumexp((1-self.alpha)*log_weights, dim=-1)
    ...
    def _sample_single_model(self) -> nn.Module:
        ...
        with torch.no_grad():
            for orig, copy_module in zip(self.bayesian_model.modules(), model_copy.modules()):
